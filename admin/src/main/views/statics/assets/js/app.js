...
function ($) {
    ...
    Topbar.prototype.changeColor = function(color) {
        var defaultTopColor = this.body.attr('data-topbar-color');
        if (defaultTopColor) {
...
function ($) {
    ...
    */
    LayoutThemeApp.prototype.applyConfig = function() {
        ...
        //设置默认的主题--chendd add
        var sidebarColor = this.body.data('sidebarColor');
        if(sidebarColor != null) {
            this.config.sidebar.color = sidebarColor;
            this.config.mode = sidebarColor;
        }
        // activate menus
