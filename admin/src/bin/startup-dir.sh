...
#项目名称
APP_NAME=`dirname $(dirname "$PWD") | awk -F "/" '{print $NF}'`
 ...
 if [ $? -eq 0 ]; then
    ...
    for (( i = 1; i <= 3; ++i )); do
        if [[ -f "$ROOT_LOG_FILE" ]]; then
            /bin/sh -c "timeout 120 tail -f $ROOT_LOG_FILE -n300"
            break
        fi
        echo "等待 $i 秒"
        sleep 1
...
checkPidNum(){
    ...
    echo "$PIDNUM = ps -ef|grep $APP_NAME|grep $APP_MAIN_CLASS|grep -v grep|awk '{print $2}'"
...
case $1 in
          ...
          restart)  restart;;
          status)  status;;
              *) echo -e "\033[41;5mrequire start|stop|close|restart|status\033[0m";;
esac
