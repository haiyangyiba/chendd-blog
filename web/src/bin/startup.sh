...
#是否启用debug模式则设置为空
#APP_DEBUGE="-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=10088"
APP_DEBUGE=""

#JVM参数  -Xms程序启动时占用内存大小 -Xmx程序运行期间最大可占用的内存大小 -Xss设定每个线程的堆栈大小   -Xmn 年轻代大小
JVM_OPTS="-Xmx1024m -Xms1024m -Xmn1024m -XX:+UseG1GC -XX:+PrintGCDetails -XX:+PrintGCApplicationStoppedTime -XX:+PrintGCTimeStamps -Xloggc:$APP_HOME/logs/gc.log -Djava.io.tmpdir=$APP_HOME/runtime -Dfile.encoding=utf-8 -Duser.timezone=GMT+8"
...
start(){
 ...
 if [ $? -eq 0 ]; then
    ...
    echo "---------------------------------"
    ROOT_LOG_FILE="$APP_HOME/logs/web/blog.root.log"
